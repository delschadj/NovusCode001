...
import 'highlight.js/styles/github.css'; // Ensure highlight.js theme is imported
...
const ChatContent: React.FC<{ onMessageSent: () => void }> = ({ onMessageSent }) => {
  ...
  const onSubmit: SubmitHandler<ChatBarFormValues> = async (data) => {
    ...
    try {
      ...
      setLoading(false);
      reset({ message: '' }); // Clear the form input
    }
  };
            ...
            {messages.map((msg, index) => (
              <div
                ...
                className={`p-3 rounded-lg max-w-full break-words overflow-hidden ${
                  msg.type === 'user'
                    ? 'bg-gray-200 text-black self-end' // User messages use gray background
                    : 'bg-transparent text-black self-start' // AI responses have transparent background
                }`}
                style={{ maxWidth: '100%' }}
              >
                <ReactMarkdown 
